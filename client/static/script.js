...
(function() {
  $(function() {
    var charge_carts, filter, insertWines, url_base;
    ...
    charge_carts = function() {
      return $.get("/getCarts", function(data) {
        var cart, carts_div, i, len, panel, ref, results, row, wine;
        carts_div = $('#carts');
        carts_div.empty();
        ref = data.all;
        results = [];
        for (i = 0, len = ref.length; i < len; i++) {
          cart = ref[i];
          row = $.parseHTML("<div class='row'> <div class='panel panel-danger'> <div class='panel-body' id='cart-" + cart.url + "'-> </div> </div> </div>");
          carts_div.append(row);
          panel = $("#cart-" + cart.url);
          panel.append("<b>" + (cart.name || 'Sin nombre') + "</b>");
          results.push((function() {
            ...
            for (j = 0, len1 = ref1.length; j < len1; j++) {
              wine = ref1[j];
              results1.push(panel.append(" - <i>" + wine + "</i>"));
            }
            return results1;
          })());
        }
